(function () {
    ...
    // ============================================
    // كلية الدكتور — يتحمل مرة واحدة عند الـ init
    // ============================================
    ...
    let _doctorCollege = null;   // e.g. "PT"
    let _collegeLetter = null;   // e.g. "P"

    const _loadDoctorCollege = async () => {
        if (_doctorCollege) return; // مش محتاج يجيبها تاني
        try {
            ...
            if (snap.exists()) {
                ...
                console.log(`🏫 GroupSearch: College = ${_doctorCollege} | Letter = ${_collegeLetter}`);
            }
        } catch (e) {
            console.warn("GroupSearch: College fetch failed", e);
        }
    };

    // ============================================
    // بناء الـ regex ديناميكياً حسب حرف الكلية
    // مثال PT → /^\dP\d{1,3}$/ أو /^\dP\d{1,3} GP$/
    // ============================================
    const _buildGroupPattern = () => {
        const L = _collegeLetter || '[A-Z]';
        // يقبل: 1P1 | 1P12 | 1P1 GP | 1P12 GP
...
.gs-back-btn {
    ...
    cursor: pointer; transition: all 0.15s; margin: 10px 18px 0; width: fit-content;
}
.gs-back-btn:hover { background: #e2e8f0; }
.gs-student-row {
    display: flex; align-items: center; padding: 11px 18px;
    border-bottom: 1px solid #f1f5f9; gap: 12px; transition: background 0.1s;
}
...
@keyframes gsSlideIn {
    ...
    const fmtDate = (isoStr) => { const [y, m, d] = isoStr.split('-'); return `${d}/${m}/${y}`; };
    ...
    const renderSingleSubject = (groupCode, targetDate, masterList, attendanceMap, subjectName, doctorName, multiSubject = false) => {
            ...
            <div class="gs-detail-view">
                ...
                <div style="padding: 10px 18px 4px; display:flex; align-items:center; gap:12px;">
                    ...
                    <div class="gs-percent-bar-wrap" style="flex:1; width:auto;">
                        <div class="gs-percent-bar-fill" style="width:0%; background:${barColor};"></div>
                    </div>
        ...
        setTimeout(() => {
            container.querySelectorAll('.gs-percent-bar-fill').forEach(bar => { bar.style.width = pct + '%'; });
        }, 50);
    };

    // ============================================
    // performSearch — مع فلتر الكلية
    // ============================================
    const performSearch = async () => {
        ...
        // ✅ الـ pattern ديناميكي حسب حرف الكلية
        const groupPattern = _buildGroupPattern();
        if (!groupPattern.test(groupCode)) {
            const example = _collegeLetter ? `مثال صحيح: 1${_collegeLetter}2` : 'تحقق من كود الجروب';
            if (typeof showToast === 'function') showToast(`⚠️ صيغة غير صحيحة. ${example}`, 3000, '#ef4444');
            input?.focus();
            return;
        }
        ...
        try {
            ...
            // ── 1. جلب قائمة الطلاب (مع فلتر الكلية) ──
            let masterList = [];

            // محاولة أولى: user_registrations
            ...
            usersSnap.forEach(d => {
                ...
                if (info.studentID) {
                    masterList.push({ id: String(info.studentID).trim(), name: info.fullName || 'غير معروف', uid: d.id });
                }
            });

            // محاولة ثانية: students collection
            ...
            attSnap.forEach(d => {
                ...
                if (sid) subjectsMap[subj].attendanceMap.set(sid, {
                    name: data.name || '', subject: subj, doctorName: doctor,
                    time_str: data.time_str || '--:--', hall: data.hall || '—', group: data.group || groupCode
                });
            });

            // Fallback: لو group field مش متسجل → ابحث بالـ student IDs
            if (Object.keys(subjectsMap).length === 0 && masterList.length > 0) {
                ...
                for (const chunk of chunks) {
                    ...
                    if (_doctorCollege) fbConstraints.push(where('college', '==', _doctorCollege));

                    const chunkSnap = await getDocs(query(collection(db, 'attendance'), ...fbConstraints));
                    chunkSnap.forEach(d => {
                        const data = d.data();
                        const sid = String(data.id || '').trim();
                        const subj = (data.subject || '—').trim();
    ...
    window._gsOpenSubject = function (subjectName) {
        ...
        if (!info) return;
        renderSingleSubject(groupCode, targetDate, masterList, info.attendanceMap, subjectName, info.doctorName, true);
    ...
    window.gsExportExcel = (groupCode, targetDate, subjectName) => {
        ...
        if (!rows.length) { if (typeof showToast === 'function') showToast('⚠️ لا توجد بيانات للتصدير', 2500, '#f59e0b'); return; }
        ...
        for (let R = 1; R <= range.e.r; R++) {
            const statusCell = XLSX.utils.encode_cell({ r: R, c: 7 });
        ...
        const colSuffix = _doctorCollege ? `_${_doctorCollege}` : '';
        XLSX.writeFile(wb, `حضور_${safeName}${colSuffix}_${safeSubj}_${safeDate}.xlsx`);
    ...
    window.gsExportCSV = (groupCode, targetDate, subjectName) => {
        ...
        const subj = subjectName || window._gsLastData._activeSubject;
        const rows = buildExportRows(groupCode, targetDate, subj);
        if (!rows.length) { if (typeof showToast === 'function') showToast('⚠️ لا توجد بيانات', 2500, '#f59e0b'); return; }
    ...
    // ============================================
    // initGroupSearchModule — مع تحميل الكلية أولاً
    // ============================================
    window.initGroupSearchModule = async () => {
        ...
        if (badgeBox && _doctorCollege) {
            badgeBox.innerHTML = `
                <div class="gsb-college-badge">
                    <i class="fa-solid fa-building-columns"></i>
                    College: ${_doctorCollege}
                </div>`;
            // تحديث الـ placeholder ليعكس حرف الكلية
            const codeInput = document.getElementById('groupCodeInput');
            if (codeInput && _collegeLetter) {
                codeInput.placeholder = `e.g. 1${_collegeLetter}2 or 1${_collegeLetter}1 GP`;
            }
        }
        ...
        if (codeInput) {
            ...
            // ✅ فلتر الإدخال يقبل الحرف الصح بس
            codeInput.addEventListener('input', () => {
                const L = _collegeLetter || 'A-Z';
                const regex = new RegExp(`[^0-9${L}${L.toLowerCase()} ]`, 'g');
